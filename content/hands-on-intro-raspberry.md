...
A Servient provides a protocol agnostic runtime for the application using the resource model. Servient application uses this resource model which enables interoperability with other Serviants and users that are using multiple bindings. Servient may act at the same time as a client and a server according to the applications' behavior. The resources associated to a Servient instance are described in a Thing description. It uses JSON-LD to make this description readable by both humans and machines. Such a description enhances the machine to machine communication and makes the things ready for the Semantic Web.
...
We assume that the Raspberry Pi used is a freshly installed one with Debian. First, it needs to be configured for a network connection. Second, the environment needs to be configured mainly by installing NodeJS and a few other dependencies. Finally, things-web needs to be installed on the system.
...
The installation process is composed of mainly two steps. The first step installs the nodeJS dependences. The second step installs and builds the node.wot project.

#### Installing the packages

Node.wot requires dependencies such as Typescript, lerna and etc. By running the command below, those dependencies are installed:
...
As the node.wot project is mainly developed using Typescript, "tsc" needs to be executed to trans-compile that code to JavaScript. The command bellow builds the project and its packages:
...
The full list of the elements that can be used in a TD is available in the W3C draft document at https://www.w3.org/TR/wot-thing-description/.
...
Web of Things programming model is based on the notion of "Servient". A Servient is a program developed using the WoT scripting API and can behave at the same time as a server and a client.
...
Now that we have our property exposed, we can start adding our actions. An action is added to Serviant as described below:
